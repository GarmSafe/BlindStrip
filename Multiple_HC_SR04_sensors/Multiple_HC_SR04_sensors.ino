...
*/
unsigned long ping_timer[SONAR_NUM][3];
/**
  Where the ping distances are stored.
*/
unsigned int cm[SONAR_NUM][3];
/**
...
void loop() {
  for (uint8_t i = 0; i < SONAR_NUM; i++) { //Loop through all the sensors.
    ...
    if (current_sensor == sensor_number) //Sensor that recognises irregularities
    {
...
int get_actual_value(int values[3]) {
  int zero_count = 0;
  int actual_value = 0;
...
void set_ping_interval() {
  ...
  for (uint8_t i = 1; i < SONAR_NUM; i++)   //Set the starting time for each sensor.
  {
    for (uint8_t j = 1; j < 3; j++) {
      ping_timer[i][j] = ping_timer[j - 1] + PING_INTERVAL;
    }
  }
}
